...
func TestVec_Pipe(t *testing.T) {
	...
	vec2 := vec.Pipe(func(i int) (int64, bool) {
		return int64(i + 1), true
	})

	expected := []int64{2, 3, 4, 5, 6, 7}
	if !vec2.Equal(hs.New(expected), hs.Eq) {
		t.Fatal("Pipe failed.")
	}
}
...
func TestVec_Contains(t *testing.T) {
	vec := hs.NewWith(1, 2, 3, 4, 5, 6)
	if !vec.Contains(hs.EqTo(3)) {
		t.Fatal("Contains failed.")
	} else if vec.Contains(hs.EqTo(8)) {
		t.Fatal("Contains failed.")
	}
}
